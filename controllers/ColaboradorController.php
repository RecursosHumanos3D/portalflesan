...
class ColaboradorController extends Controller {
    ...
    public function actionMatch($rut){
    ...
    public function actionMatchamigo($rut){
     ini_set('memory_limit', '8000M');
     ini_set('max_execution_time', 800);
  ...
  public static function findCumple(){
     $connection = Yii::$app->db;
        $count = Yii::$app->db->createCommand('select CONCAT(nombreColaborador, " ", apellidosColaborador) AS nombre, rutColaborador, r.rfoto from colaborador c inner join rperfilredsocial r on c.idperfilRed=r.idperfilRed where DAY(fechanacimiento) = DAY(NOW()) AND MONTH(fechanacimiento) = MONTH(NOW());')
        ->queryAll();
   ...
   public static function findProxCumple(){
     $connection = Yii::$app->db;
        $count = Yii::$app->db->createCommand('select CONCAT(nombreColaborador, " ", apellidosColaborador ) AS nombre , rutColaborador, day(fechanacimiento) as dia,  MONTH(fechanacimiento) AS mes, r.rfoto  FROM colaborador c inner join rperfilredsocial r on c.idperfilRed=r.idperfilRed WHERE  DATE_ADD(fechanacimiento, 
                INTERVAL YEAR(CURDATE())-YEAR(fechanacimiento)
                         + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(fechanacimiento),1,0)
                YEAR) 
            BETWEEN CURDATE() AND DATE_ADD(CURDATE(), INTERVAL 7 DAY)
     AND DATE_ADD(fechanacimiento, INTERVAL YEAR(CURDATE())-YEAR(fechanacimiento) YEAR) <> CURDATE() order by ( DAYOFYEAR(fechanacimiento) - (DAYOFYEAR(CURDATE()) )) asc
     ;
')
        ->queryAll();
   ...
   public static function findNextMonth(){
     $connection = Yii::$app->db;
        $count = Yii::$app->db->createCommand('select CONCAT(nombreColaborador, " ", apellidosColaborador ) AS nombre , rutColaborador, day(fechanacimiento) as dia,  MONTH(fechanacimiento) AS mes, r.rfoto  
FROM colaborador c inner join rperfilredsocial r on c.idperfilRed=r.idperfilRed  WHERE  DATE_ADD(fechanacimiento, 
                INTERVAL YEAR(CURDATE())-YEAR(fechanacimiento)
                         + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(fechanacimiento),1,0)
                YEAR) 
            BETWEEN DATE_ADD(CURDATE(), interval 7 day) AND DATE_ADD(CURDATE(), INTERVAL 1 Month)
     AND DATE_ADD(fechanacimiento, INTERVAL YEAR(CURDATE())-YEAR(fechanacimiento) YEAR) <> CURDATE() order by ( DAYOFYEAR(fechanacimiento) - (DAYOFYEAR(CURDATE()) )) asc;
')
        ->queryAll();
   ...
   public static function findAllBirthday(){
     $connection = Yii::$app->db;
        $count = Yii::$app->db->createCommand('select CONCAT(nombreColaborador, " ", apellidosColaborador ) AS nombre , rutColaborador, day(fechanacimiento) as dia,  MONTH(fechanacimiento) AS mes, r.rfoto  FROM colaborador c inner join rperfilredsocial r on c.idperfilRed=r.idperfilRed WHERE  DATE_ADD(fechanacimiento, 
                INTERVAL YEAR(CURDATE())-YEAR(fechanacimiento)
                         + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(fechanacimiento),1,0)
...
')
  ...
  public function actionCumpleanios(){
    $session = Yii::$app->session;
        ...
        if ($rutColaborador == null) {
            ...
            $colaboradores = $this->findCumple();
            $todos = $this->findAllBirthday();
            $proxmes = $this->findNextMonth();
            $proximos = $this->findProxCumple();
        ...
        return $this->render('cumpleanios', [
             'model' => $model,
  ...
  public function actionContenidos($bibIdContenido){
     $session = Yii::$app->session;
        $rutColaborador = $session['rut'];
        ...
        if ($rutColaborador == null) {
            ...
            $model = BuscarController::encuentraContenido($bibIdContenido);
            $contenido =  \app\models\BibContenido::find()->where(['bibIdContenido' => $bibIdContenido])->one();
            $contenido->bibVisita = $contenido->bibVisita + 1;
            $contenido->save(false);
            
             return $this->render('contenidoHome', [
                    'model' => $model,
                  ]); 
  ...
  public function actionBiblioteca(){
        ...
        $contenidos = BuscarController::encuentraTodobib();


            return $this->render('bibliotecaHome', [
    ...
    public function actionPerfil() {
        ...
        $colaboradores = $this->findCumple();
        $comunicaciones = BuscarController::findComunicaciones();
        $proximos = $this->findProxCumple();
     
        //var_dump($proximos);die();
    ...
    */
     ...
     public function actionFoto($rutColaborador) {
        try {
            ...
            $model = BuscarController::findColaboradorRut($rutColaborador);
            $perfil = BuscarController::findPerfil($model->idperfilRed);
            //Muestra los hobbies actuales del Colaborador

          //  var_dump(Yii::$app->request->post()["Rperfilredsocial"]["rpasatiempo"]);die();
            if ($perfil->load(Yii::$app->request->post())) {
                ...
                if (empty($perfil->file)) {
                    ...
                    $perfils->rmusica =$perfil->rmusica;
                    $perfils->rhobbie =$perfil->rhobbie;
                    $models->save(false);
                    $perfils->save(false);
                } else {
                ...
                return $this->redirect(['colaborador/compadre', 'rutAmigo' =>  $model->rutColaborador]);
            } else {
  ...
  public function actionContenido($page, $rutColaborador){
                     ...
                     \Yii::$app->getSession()->setFlash('error', ' <div class="col-sm-12 col-md-12">
                        <div class="alert alert-danger">
                           ...
                           <span class="glyphicon glyphicon-no"></span> <strong>Mensaje de error</strong>
                            ...
                            <p>
                                Debe ingresar alg̼n contenido a postear.</p>
                        </div>
                    </div>');
               ...
               //var_dump($model->file[0]);die();
                ...
                if ($model->file[0]->type == "application/msword" || $model->file[0]->type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document") {
                    ...
                    foreach ($model->file as $file) {
                        ...
                        $model->rdescArchivo = Yii::$app->request->post()["rdescripcionPost"];
                ...
                if ($model->file[0]->type == "application/vnd.openxmlformats-officedocument.presentationml.presentation") {
                    ...
                    foreach ($model->file as $file) {
                        ...
                        //$model->rdescripcionPost = $model->rut1 . $file->baseName . $num . "." . $file->extension;
                        $model->rdescArchivo = Yii::$app->request->post()["descripcionPost"];
                        $model->rnombreArchivo = $file->baseName . "." . $file->extension;
                        $model->rdescArchivo = Yii::$app->request->post()["rdescripcionPost"];
                    }
                }
                if ($model->file[0]->type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" || $model->file[0]->type == "application/vnd.ms-excel") {
                    $model->rtipoPost = 6; // este post es con foto
                    foreach ($model->file as $file) {
                        $file->saveAs('img/archivos/' . $model->rut1 . $file->baseName . $num . "." . $file->extension);
                        $ruta = 'img/archivos/' . $model->rut1 . $file->baseName . $num . "." . $file->extension;
    ...
    public function actionCompadre($rutAmigo) {
        ...
        if($rutColaborador==null){
            ...
            $fotos = BuscarController::findFotos($rutAmigo);
            $top = BuscarController::findTopFotos($rutAmigo);
            $others = BuscarController::findOthers($rutAmigo);
            $last = BuscarController::findLast($rutAmigo);
            $model = BuscarController::findColaboradorRut($rutAmigo);
            $model2 = BuscarController::encuentraAmigos($rutAmigo);
            $cargo = BuscarController::findCargo($model->idCargo);
            $hobbie = BuscarController::findHobbiesTipo($model->idperfilRed);
   
              //var_dump($hobbie);die();
           
            $perfil = BuscarController::findPerfil($model->idperfilRed);
            $actividad = BuscarController::findMuroa($rutAmigo);
            $antiguedad= BuscarController::findAntiguedad($rutAmigo);

            $meses = $antiguedad/12;

            $entero = (int) $meses;  // Entero
            $frac  = $meses - (int) $meses;
            if ($frac == 0){
                $decimal2 = 0;
            }else{
                $frac2 = round($frac, 2);
                $decimal = substr($frac2, 2);
                $decimal2 = substr($decimal, 1);
            }
            //var_dump($entero);die();
            $model3 = new RPost();
            $estadistica = BuscarController::findEstadistica($model->idestadisticas);
    ...
    public function actionReload($page, $rutColaborador){
        ...
        foreach($actividad as $rpost){
             ...
             if ($rpost["rtipoPost"] == 1) {
               ...
               $modelo = $this->renderAjax('estado2', [
                              'model' => $model,
                              'post' => $rpost,
                              'perfil' => $perfil,
                              'perfil2' => $perfil2,
     ...
     public function actionReloadr($page, $rutColaborador, $rutAmigo){
        ...
        $perpage = 3;
        $posisi = (($numpage-1) * $perpage);
        $actividad = BuscarController::findMurora($rutAmigo, $posisi, $perpage, $rutAmigo);
        $model = BuscarController::findColaboradorRut($rutColaborador);
        $total = "";
        $session = Yii::$app->session;
        $rutColaborador = $session['rut'];
    ...
    public function actionConvenios(){
        ...
        $categorias = BuscarBeneficiosController::buscarCategoriaBeneficio();

        return $this->render('beneficiosFlesan', [
    ...
    public function actionAmigos(){
        ...
        return $this->render('listadoHome', [
    ...
    public function actionVerConvenios($idbc){
         $session = Yii::$app->session;
        $rutColaborador = $session['rut'];
        //var_dump($rutColaborador);die();
        if ($rutColaborador == null) {
      ...
      $rutColaborador = $session['rut'];
        ...
        if ($rutColaborador == null || $rutColaborador != $rutAmigo) {
            ...
            $publicidad = BuscarController::findPublicidad();
            $puntajeUsado = BuscarController::findPuntosUsados($rutColaborador);
            $suma = 0;
           
            foreach ($puntajeUsado as $p) {
               $suma = $suma + $p->bvalorCanje;
      ...
      return $this->render('puntosHome', [
        ...
        public function actionValidar($rutColaborador, $idbeneficio) {
...
return 1; // se canjeo
     ...
     public function actionSolicitar($rutColaborador, $idbeneficio, $fecha, $jornada) {
        ...
        date_default_timezone_set("America/Santiago");
            ...
            $anio = BuscarController::canjeA($rutColaborador, $beneficio["bId_Beneficio"]);
                        ...
                        ->setFrom(["rrhh@flesan.cl"=>"PORTAL FLESAN"])
...
ha canjeado un nuevo beneficio llamado:&nbsp;<strong>'.$beneficio->bNombre.'</strong>&nbsp;con un valor de: '.$beneficio->bValorBeneficio.' puntos. Jornada: '.$canje->bjornada.' con fecha '.$canje->bfechatoma.'.</span><br>
&nbsp;</div>
                    ...
                    <tbody>
                        <tr>
                            <td align="center" valign="middle" class="mcnButtonContent" style="font-family: Arial; font-size: 16px; padding: 15px;">
                                <a class="mcnButton " title=" IR A PORTAL FLESAN" href="https://portalflesan.cl" target="_blank" style="font-weight: bold;letter-spacing: normal;line-height: 100%;text-align: center;text-decoration: none;color: #FFFFFF;"> IR A PORTAL FLESAN</a>
                            </td>
                        </tr>
                    </tbody>
                </table>
      ...
      <table align="left" border="0" cellpadding="0" cellspacing="0" width="100%">
        <tbody>
          <tr>
            ...
            <a href="mailto:rrhh@flesan.cl">rrhh@flesan.cl</a><br>
            <br>
