...
<<<<<<< HEAD
    public function actionCompartir($idPost, $rutColaborador){
        $actividad = BuscarController::findActividad($idPost);
        $post = BuscarController::findPost($idPost);

        $postCompartido = new Rpost();
        $postCompartido->rdescripcionPost = $post->rdescripcionPost;
        $postCompartido->rfoto = $post->rfoto;
        $postCompartido->rfecha = $post->rfecha;
        $postCompartido->rtipoPost = $post->rtipoPost;
        $postCompartido->rrotador = $post->rrotador;
        $postCompartido->rnombreArchivo = $post->rnombreArchivo;
        $postCompartido->rut1 = $rutColaborador;
        $postCompartido->rut2 = 1;
        $postCompartido->grupo = 1;
        $postCompartido->save(false);

        $postCompartidoActividad = new RActividad();
        $postCompartidoActividad->rutColaborador1 = $rutColaborador;
        $postCompartidoActividad->rutColaborador2 = 1;
        $postCompartidoActividad->ridpost = $postCompartido->ridPost;
        $postCompartidoActividad->ridtipo_post = $actividad->ridtipo_post;
        $postCompartidoActividad->save(false);
